...
UIntControl::~UIntControl()
{
}

bool UIntControl::valueChanged()
{
	unsigned int newValue = m_intSpin->value();
...
void UIntControl::refreshFromValue()
{
	m_intSpin->setValue(*m_pairedValue);
}

bool UIntControl::deltaChange(float delta, unsigned int index)
{
	bool changed = false;
	int iDelta = (int)delta;
	// need to check the bounds.
	unsigned int currentValue = *m_pairedValue;
	if (iDelta > 0)
	{
