...
void Hash::addLongLong(long long value)
{
	addData((const unsigned char*)&value, sizeof(long long));
}

void Hash::addFloat(float value)
{
	int e;
	double f = frexp(value, &e);
	long long m = ldexp(f, 63);

	addInt(e);
	addLongLong(m);
}

void Hash::addFloatFast(float value)
{
	addData((const unsigned char*)&value, sizeof(float));
}
...
void Hash::addPoint(const Point& point)
{
	addFloat(point.x);
	addFloat(point.y);
	addFloat(point.z);
...
void Hash::addVector(const Vector& vector)
{
	addFloat(vector.x);
	addFloat(vector.y);
	addFloat(vector.z);
...
HashValue Hash::hashPoint(const Point& point)
{
	Hash newHash;
	newHash.addPoint(point);
...
HashValue Hash::hashData(const unsigned char* buffer, unsigned int length)
{
	HashValue crc = 0;

	while (length--)
	{
		crc = kCRCTab64[(crc ^ *(buffer++)) & 0xff] ^ (crc >> 8);
	}
